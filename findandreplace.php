...
//Параметры
define('PASS', '123'); //пароль (желательно поменять на свой). Для запуска скрипта /findandreplace.php?pass=123
	...
	while (($file = readdir($dir)) !== false) {
		...
		if ($file != '.' && $file != '..') {
			...
			if (is_file($file_name)) {
				...
				if ($regex) {
					if (preg_match('/'.$text.'/'.$regis, $content, $res, PREG_OFFSET_CAPTURE)) {
						$str = preg_replace('/('.$text.')/'.$regis, "%find%\$1%/find%", mysubstr($content, $res[0][1], $res[0][0]));
					}
				} else {
...
function replace($content, $file_name, $regex) {
	...
	if ($regex) {
		$content = preg_replace('/'.$text.'/'.$regis, $retext, $content);
	} else {
...
if (isset($_POST['all'])) {
	...
	if (is_file($file) == true) {
		...
		if ($_POST['regex_e']) {
			$content = preg_replace('/('.$text.')/'.$_POST['regis_e'], "%find%\$1%/find%", $content);
		} else {
            ...
            <tr>
                ...
                <td>
                    <textarea name="retext"><?php echo isset($retext) ? $retext : ''; ?></textarea>
						  <label style="padding-left: 20px;">Замена</label><input type="checkbox"<?php echo isset($replace) && $replace == 1 ? ' checked' : ''; ?> name="replace" value="1" />
                </td>  
            </tr>
            <tr>
                <td align="right">
                ...
                <td>
                    <input style="width:97%" type="text" name="ext" value="<?php echo isset($_POST['ext']) ? $_POST['ext'] : EXT; ?>" />
                </td>
            </tr>
